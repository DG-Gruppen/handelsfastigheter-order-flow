...
function buildOrgTree(profiles: OrgProfile[], roleMap: RoleMap): OrgNode | null {
  ...
  function toNode(profile: OrgProfile, forceType?: NodeType): OrgNode {
    ...
    return {
      ...
      children: children.map(c => {
        // Direct children of root without own children → staff type
        if (type === "root" && (childrenByManager.get(c.id) ?? []).length === 0
            && roleMap[c.user_id] !== "admin" && roleMap[c.user_id] !== "manager") {
  ...
  // Find primary root (prefer admin)
  ...
  // Other orphan roots become children of primary root
  const otherRoots = roots.filter(r => r.id !== primaryRoot.id);
...
export default function OrgTree() {
  ...
  const handleMove = useCallback(async (movedNodeId: string, newParentId: string) => {
    ...
    if (error) {
      ...
      fetchData(); // Refetch on error to reset
    } else {
