...
// ─── Color palette ──────────────────────────────────────────
const C: Record<ColorKey, { a: string; b: string; t: string; g: string; r: string; n: string }> = {
  ...
  return (
    <g style={{ cursor: isRoot ? "default" : "grab", opacity: dragging ? 0.15 : 1, transition: "opacity 0.2s" }}
      ...
      <text x={textX} y={y + h / 2 - (isStaff ? 7.5 : 9)} fontFamily="'Space Grotesk',sans-serif" fontWeight="700"
        fontSize={isRoot ? 13 : isStaff ? 11 : 11.5} fill={isStaff ? "#c4b5fd" : "#dde8f5"}>
    ...
    <div style={{
      ...
      <svg width={w + 16} height={h + 16} style={{ overflow: "visible" }}>
        ...
        <text x={8 + 44} y={8 + h / 2 - 8} fontFamily="'Space Grotesk',sans-serif" fontWeight="700" fontSize={11}
          fill={node.type === "staff" ? "#c4b5fd" : "#dde8f5"}>{node.position.slice(0, 18)}</text>
...
function Toolbar({ onExpAll, onCollAll, total }: {
  ...
  return (
    <div style={{
      ...
      <span style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 10.5, color: "#5a7a9a", fontWeight: 600 }}>
        {total} noder
      </span>
      ...
      {legs.map(({ c, l, d }) => (
        <div key={c} style={{ display: "flex", alignItems: "center", gap: 5 }}>
          ...
          <span style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 10, color: "#5a7a9a", fontWeight: 500 }}>{l}</span>
        </div>
      ))}
    </div>
  );
...
export default function OrgChartCanvas({ initialTree, onMoveNode }: OrgChartCanvasProps) {
  ...
  const onFit = () => {
    ...
    setPan({ x: (vp.width - svgW * z) / 2, y: (vp.height - svgH * z) / 2 });
    setZoom(z);
  };
  ...
  const findDrop = useCallback((sx: number, sy: number, dragId: string) => {
  ...
  // Mouse move / up for drag and pan
  useEffect(() => {
    const onMove = (e: globalThis.MouseEvent) => {
      ...
      const dt = findDrop(e.clientX, e.clientY, u.id);
  ...
  const snapLine = useMemo(() => {
    ...
    const pos = positions.get(drop); if (!pos) return null;
    const { x: sx, y: sy } = screenToSvg(drag.curX, drag.curY);
    const tx = pos.x + pos.w / 2, ty = pos.y + pos.h, midY = ty + (sy - ty) / 2;
    const dropNode = findNode(tree, drop);
    return {
      d: `M${tx},${ty}L${tx},${midY}L${sx},${midY}L${sx},${sy}`,
      accent: C[dropNode?.color ?? "cyan"].a,
    };
        ...
        <div style={{
          ...
          <svg ref={svgRef} width={svgW} height={svgH} viewBox={`${minX} ${minY} ${svgW} ${svgH}`}
            ...
            {snapLine && (
              <g>
                ...
                <path d={snapLine.d} fill="none" stroke={snapLine.accent} strokeWidth={1.5}
                  ...
                  style={{ animation: "dashAnim 0.3s linear infinite" }} />
                {(() => {
                  const dropPos = positions.get(drop!);
                  if (!dropPos) return null;
                  return (
                    <circle cx={dropPos.x + dropPos.w / 2} cy={dropPos.y + dropPos.h} r={3.5} fill={snapLine.accent}>
            ...
            {Array.from(positions.entries()).map(([id, pos]) => {
              ...
              if (!node) return null;
              return <SvgCard key={id} node={node} pos={pos} dragging={ghostIds.has(id)}
                isDrop={drop === id} onMD={(e) => onCardDown(e, id)} />;
            })}
            ...
            {Array.from(positions.entries()).map(([id, pos]) => {
              ...
              return <CollapseBtn key={`cb-${id}`} pos={pos} collapsed={coll.has(id)}
                count={countDesc(node)} color={node.color} onClick={() => toggle(id)} />;
            })}
          </svg>
        </div>
