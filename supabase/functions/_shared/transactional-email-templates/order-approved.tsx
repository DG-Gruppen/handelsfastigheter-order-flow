...
const OrderApprovedEmail = ({
  ...
  const approvalText = isAutoApproved
    ? `Din beställning "${title}" har godkänts automatiskt och skickats vidare till IT för hantering.`
    : `Din beställning "${title}" har godkänts av ${approverName} och skickats vidare till IT för hantering.`

  return (
    <Html lang="sv" dir="ltr">
      <Head />
      <Preview>Din beställning har godkänts: {title}</Preview>
      <Body style={main}>
        <Container style={container}>
          <div style={header}>
            <Heading style={headerTitle}>✅ Beställning godkänd</Heading>
          </div>
          <div style={content}>
            <Text style={greeting}>
              Hej <strong style={{ color: BRAND.textDark }}>{recipientName}</strong>,
            </Text>
            <Text style={text}>{approvalText}</Text>
            {items.length > 0 && (
              <>
                <Heading style={sectionHeading}>Beställd utrustning</Heading>
            ...
            <div style={btnWrap}>
              <Button style={button} href={orderUrl}>Visa din beställning</Button>
...
export const template = {
  component: OrderApprovedEmail,
  subject: (data: Record<string, any>) =>
    `[SHF IT] Din beställning har godkänts: ${data.title || 'Beställning'}`,
  displayName: 'Beställning godkänd',
  previewData: {
